...
// 하늘 생성
const skyGeometry = new THREE.SphereGeometry(1000, 0, 0); // 전체를 덮을 큰 구를 만듭니다.
...
// 경기장 가장자리 벽
const wallGeometry1 = new THREE.BoxGeometry(0.1, 15, 18); // 경기장 크기에 맞게 조정할 것
const wallGeometry2 = new THREE.BoxGeometry(9, 15, 0.1); // 경기장 크기에 맞게 조정할 것
const wallGeometry3 = new THREE.BoxGeometry(9, 0.1, 18); // 경기장 크기에 맞게 조정할 것
...
leftWall.position.set(-courtWidth / 2 - 0.01, 5, 0); // 왼쪽 경기장 가장자리 위치
const rightWall = new THREE.Mesh(wallGeometry1, wallMaterial);
rightWall.position.set(courtWidth / 2 + 0.01, 5, 0); // 오른쪽 경기장 가장자리 위치
const backWall = new THREE.Mesh(wallGeometry2, wallMaterial);
backWall.position.set(0, 5, -courtHeight / 2 - 0.01); // 뒤쪽 경기장 가장자리 위치
const frontWall = new THREE.Mesh(wallGeometry2, wallMaterial);
frontWall.position.set(0, 5, courtHeight / 2 + 0.01); // 앞쪽 경기장 가장자리 위치
...
function handleBounce(ball, object) {
  ...
  const newVelocity = direction.multiplyScalar(speed); // 벽일 때만 바운스 속도 감소
